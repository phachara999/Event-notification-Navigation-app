...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($first_name) || empty($last_name) || empty($faculty_id) || empty($branch_id)) {
    ...
    // Prepare SQL query
    $sql = "INSERT INTO personnel (first_name, last_name, faculty_id, branch_id)
            VALUES (?, ?, ?, ?)";
    
    // Prepare parameters
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssii", $first_name, $last_name, $faculty_id, $branch_id);
