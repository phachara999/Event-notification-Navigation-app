...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($id) || empty($first_name) || empty($last_name) || empty($faculty_id) || empty($branch_id)) {
    ...
    $sql = "UPDATE personnel 
            SET first_name = ?, last_name = ?, faculty_id = ?, branch_id = ?
            WHERE id = ?";
    
    // Prepare parameters
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssiii", $first_name, $last_name, $faculty_id, $branch_id, $id);
    // Execute SQL query
    if ($stmt->execute()) {
